...
type HTTPOutputSwConfig struct {
	...
	SwApi          string        `json:"output-http-sw-api"`
...
func NewHTTPSwOutput(address string, config *HTTPOutputConfig) PluginReadWriter {
	...
	o.client = NewHTTPClientSw(o.config)
...
func (o *HTTPOutputSw) workerMaster() {
...
func (o *HTTPOutputSw) startWorker() {
	for {
		select {
		case <-o.stopWorker:
			return
...
func (o *HTTPOutputSw) PluginWrite(msg *Message) (n int, err error) {
...
func (o *HTTPOutputSw) PluginRead() (*Message, error) {
	if !o.config.TrackResponses {
...
func (o *HTTPOutputSw) sendRequest(client *HTTPClientSw, msg *Message) {
	if !isRequestPayload(msg.Meta) {
		return
	}
	...
	resp, err := client.SendSwApi(buildRequestBody(o.config.SwSource, mimeHeader, meta, req))
	stop := time.Now()
...
type HTTPClientSw struct {
	config *HTTPOutputConfig
	Client *http.Client
}

// NewHTTPClient returns new http client with check redirects policy
func NewHTTPClientSw(config *HTTPOutputConfig) *HTTPClientSw {
	client := new(HTTPClientSw)
...
func (c *HTTPClientSw) Send(data []byte) ([]byte, error) {
...
// Send sends an http request using client create by NewHTTPClient
func (c *HTTPClientSw) SendSwApi(body []byte) ([]byte, error) {
	var req *http.Request
	var resp *http.Response
	var err error
	req, err = http.NewRequest("POST", c.config.SwApi, bytes.NewBuffer(body))
	if err != nil {
		return nil, err
	}
	// we don't send CONNECT or OPTIONS request
